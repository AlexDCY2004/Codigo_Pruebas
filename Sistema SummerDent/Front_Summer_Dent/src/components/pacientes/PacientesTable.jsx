...
const formatDate = (value) => {
  if (!value) return '-';
  try {
    return new Date(value).toLocaleDateString('es-EC', { year: 'numeric', month: '2-digit', day: '2-digit' });
  } catch {
    return '-';
  }
};
...
export default function PacientesTable({ pacientes, onEdit, onDelete, onView, isLoading }) {
  ...
  const togglePacienteDetails = (pacienteId) => {
    setExpandedPacienteId((currentId) => (currentId === pacienteId ? null : pacienteId));
  };
  ...
  return (
    <div className="table-container">
          ...
          {pacientes.map((paciente) => {
            ...
            return (
              <tr key={paciente.id_cedula}>
                ...
                <td>{formatDate(paciente.created_at)}</td>
      ...
      <div className="patients-mobile-list" aria-label="Tabla de pacientes en móvil">
        {pacientes.map((paciente) => {
          const nombre = `${paciente.nombre || ''} ${paciente.apellido || ''}`.trim() || '-';
          ...
          return (
            <article key={paciente.id_cedula} className="patients-mobile-card">
              <button
                type="button"
                className="patients-mobile-card__summary"
                onClick={() => togglePacienteDetails(paciente.id_cedula)}
                aria-expanded={isExpanded}
                aria-controls={`patient-mobile-details-${paciente.id_cedula}`}
              >
                <span className="patients-mobile-card__toggle" aria-hidden="true">
                  {isExpanded ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                </span>
                <span className="patients-mobile-card__summary-main">
                  <span className="patients-mobile-card__name-label">Nombre</span>
                  <span className="patients-mobile-card__name">
                    <strong>{nombre}</strong>
                  </span>
                </span>
                <span className="patients-mobile-card__summary-actions" onClick={(event) => event.stopPropagation()}>
                  {renderPacienteActions(paciente)}
                </span>
              </button>

              {isExpanded ? (
                <div className="patients-mobile-card__details" id={`patient-mobile-details-${paciente.id_cedula}`}>
