...
export default function DashboardPage() {
  ...
  const [upcomingMode, setUpcomingMode] = useState('total');
  ...
  const dashboardSummary = [
    { id: 'citas-hoy', title: 'Citas de Hoy', value: data?.summary?.citasHoy ?? 0 },
    { id: 'total-ingresos', title: 'Total Ingresos', value: currency.format(data?.summary?.totalIngresos ?? 0) },
    { id: 'total-doctores', title: 'Doctores Activos', value: data?.summary?.totalDoctoresActivos ?? 0 }
  ];
  ...
  const upcomingAppointments = nextAppointments.filter((appointment) => {
    ...
    if (upcomingMode === 'manana') return appointmentDate === tomorrow;
    if (upcomingMode === 'proximo-mes') return appointmentDate.startsWith(nextMonthKey);
    return true;
  });
  ...
  if (isError) {
    return (
      <ErrorState
        ...
        message="Verifica la sesión o la conexión e intenta nuevamente."
        onRetry={() => refetch()}
      />
    );
  }
  ...
  return (
    <section className={`dashboard-grid mode-${mode}`}>
          ...
          {mode === 'rango' && (
            <div className="dashboard-range-picker">
              <label className="dashboard-range-field">
                <span>Desde</span>
                <input type="date" value={desde} onChange={(e) => setDesde(e.target.value)} className="dashboard-date-input" />
              </label>
              <label className="dashboard-range-field">
                <span>Hasta</span>
                <input type="date" value={hasta} onChange={(e) => setHasta(e.target.value)} className="dashboard-date-input" />
              </label>
            </div>
          )}
        </div>
      ...
      <div className="summary-grid">
        {isLoading
          ? [1, 2, 3].map((item) => (
              <article key={item} className="stat-card"><LoadingState lines={2} /></article>
            ))
          : dashboardSummary.slice(0, 4).map((item) => (
              ...
              todaysAppointments.map((appointment) => (
                <div key={`today-${appointment.id}`} className="appointment-item">
                  <div>
                    ...
                    <span>{appointment.date} - {appointment.start} a {appointment.end}</span>
                  </div>
                  <span className={appointment.status === 'confirmada' ? 'appointment-status appointment-status--ok appointment-status--today' : 'appointment-status appointment-status--pending appointment-status--today'}>
                    {appointment.status}
                  </span>
                </div>
              ))
        ...
        <article className="panel-card">
          <div className="panel-card__header panel-card__header--appointments">
            ...
            <div className="segmented-buttons dashboard-appointments-segmented" aria-label="Filtros de próximas citas">
              <button type="button" className={`seg-btn seg-btn--hoy ${upcomingMode === 'total' ? 'is-active' : ''}`} aria-pressed={upcomingMode === 'total'} onClick={() => setUpcomingMode('total')}>Total</button>
              <button type="button" className={`seg-btn seg-btn--rango ${upcomingMode === 'manana' ? 'is-active' : ''}`} aria-pressed={upcomingMode === 'manana'} onClick={() => setUpcomingMode('manana')}>Mañana</button>
              <button type="button" className={`seg-btn seg-btn--acumulado ${upcomingMode === 'proximo-mes' ? 'is-active' : ''}`} aria-pressed={upcomingMode === 'proximo-mes'} onClick={() => setUpcomingMode('proximo-mes')}>Próximo mes</button>
            </div>
          </div>
          <div className="appointments-list">
            {isLoading ? (
              ...
              upcomingAppointments.map((appointment) => (
                <div key={appointment.id} className="appointment-item">
                  ...
                  <span className={appointment.status === 'confirmada' ? 'appointment-status appointment-status--ok' : 'appointment-status appointment-status--pending'}>
                    {appointment.status}
                  </span>
                </div>
              ))
