...
async function safeSelect(table, filter = {}) {
  ...
  if (filter.desde) query = query.gte('fecha', filter.desde);
  if (filter.hasta) query = query.lte('fecha', filter.hasta);
...
export async function fetchDashboardSnapshot(opts = {}) {
  ...
  const [citas, movimientos, pacientes, doctores] = await Promise.all([
    safeSelect('cita', sedeFilter),
    safeSelect('movimiento_finanzas', { ...sedeFilter, desde, hasta }),
    safeSelect('paciente', sedeFilter),
    safeSelect('doctor', sedeFilter)
  ]);
